...
async def apply(ctx, url, meta):
  ...
  r = requests.head(url, allow_redirects=True, verify=False, headers=headers, timeout=(tc,tr))
  ...
  if ctyp == os.getenv('DOWNLOAD_TYPE'):
    ...
    with requests.get(url, allow_redirects=True, verify=False, headers=headers, timeout=(tc, tr)) as r:
      r.raise_for_status()
